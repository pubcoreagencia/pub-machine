...
5. **Zero Fake Work & Integridade da Auditoria:** Declarar como **IMPLEMENTADO** exclusivamente o que o código-fonte e os testes automatizados comprovam.
...
> Uma presença em geofence não cria um lead automaticamente. A presença gera um sinal; múltiplos sinais agregados geram inteligência de audiência; evidências consistentes geram sinais de intenção via `IntentBridge`; e o `PhysicalIntentAdapter` conecta essas intenções ao motor de leads existente sem duplicação de scoring.
...
| **Ponte de Intenção Física para Leads** | `PhysicalIntentAdapter` (`src/audience/physical-intent.adapter.ts`) | **IMPLEMENTADO (V0)** | Canal `physical` integrado ao `LeadIntentSignalsService` com decay temporal (half-life 72h) e auditoria completa (`tests/physical-intent-integration.test.ts`). | Webhooks de despacho em tempo real para automações de cadência. |
| **Sinais de Intenção Digital Multi-Canal** | `LeadIntentSignalsService` (`src/prospecting/lead-intent-signals.service.ts`) | **IMPLEMENTADO** | Modelagem cross-channel (digital + físico), decaimento temporal exponencial e saturação logística. | Persistir buffer in-memory em Redis/Postgres; plugar webhooks em tempo real. |
